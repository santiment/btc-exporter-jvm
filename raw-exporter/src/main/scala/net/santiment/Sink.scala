...
trait Sink[T] {
  ...
  def send(key:String, value:T):Unit = send(value)
  def flush()
}
...
class KafkaByteArraySink(producer:KafkaProducer[String, Array[Byte]], topic:String)
  extends TransactionalSink[Array[Byte]]
  ...
  override def send(key: String, bytes:Array[Byte]): Unit = {
